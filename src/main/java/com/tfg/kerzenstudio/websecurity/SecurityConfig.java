...
public class SecurityConfig  extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
            http
                    .authorizeRequests()
                    .antMatchers("/","login", "/index", "/mujer", "/hombre", "/accesorios", "/contacto", "/registrarse","/guardarregistrarse","/css/**","/img/**").permitAll()
                    .anyRequest().authenticated()
                    .and()   
                    .formLogin().defaultSuccessUrl("/acceso",true)
                    .and()
                    .logout().logoutSuccessUrl("/").logoutRequestMatcher(new AntPathRequestMatcher("/cerrarSesion"));
